...
public class SyntaxBuilder {
    ...
    private void process(Instruction[] code, int index, HashSet<Integer> labels) {
        ...
        if (command == POP_INT_DISCARD || command == POP_LONG_DISCARD || command == POP_STRING_DISCARD) {
            pops.add(null);
            return;
        }

        if (command == POP_VAR || command == POP_VARBIT) {
            pops.add(operand);
            return;
        }
        ...
        if (command == PUSH_STRING_LOCAL) {
            buildCommand(code, index, FLOW_LOAD, new LocalReference(LocalDomain.OBJECT, (int) operand), List.of(), List.of(Type.UNKNOWN_OBJECT));
            return;
        }
    ...
    private boolean isConditionalBranch(Command command) {
        ...
        if (command == BRANCH_IF_TRUE) return true;
        if (command == BRANCH_IF_FALSE) return true;
        return false;
    }
}
